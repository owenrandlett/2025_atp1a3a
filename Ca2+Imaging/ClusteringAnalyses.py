...
fish_ind = 7
fish_name = cluster_results[fish_ind]['fish_name']                           # current fish identifier
print(fish_name)
...
F_norm_fish = F_norm[fish_IDs, start_analyze_frame:]                                            # subset of traces for the current fish
                                          ...
                                          # neuron-level assignment cutoff

#%%
corr_threshold = 0.3 # minimum |r| to tag a cluster with a regressor
...
remaining_centroids = sorted(set(range(cluster_centroids.shape[0])) - assigned_centroids_set)
...
for k, signal_name in enumerate(combined_signals_names):
    ...
    F_norm_in_signal = F_norm[associated_neurons_idx, :]
