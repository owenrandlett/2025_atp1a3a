...
\caption{\textbf{\emph{atp1a3a} mutants show reduced directional responses to Optomotor stimuli}\\
...
To test the optomotor response in \emph{atp1a3a} mutants, we placed larvae in an arena in which they can swim freely in long rectangular channels while being presented with either global red light (No-Stimulus condition) or a striped pattern moving along the tracks alternating between upwards and downwards motion (OMR condition) (\autoref{fig:4}A). 
Larvae respond to this motion in multiple ways, including: 
...
We note also that mutants showed hypoactivity relative to WT siblings in the "No-Stimulus" condition (\autoref{fig:4}B, p=0.049, Kruskal-Wallis/Dunn's), indicating that there is a context dependence to the hyperactivity phenotype in mutants that we observed in our multi-well assay (\autoref{fig:2}A), and that perhaps the hypoactivity observed here is related to the different lighting, larger arena, or differing handling during the experiment (see Methods).
...
We observed relatively little change in the diencephalic pretecum, which is thought to be more critical than the tectum for motion processing in the context of the OMR \citep{Matsuda2021-a, Wang2020-a}, indicating that this area may be less affected by in \emph{atp1a3a} mutants.
However, suppressed activity in the anterior hindbrain (Rhombomeres 1/2) which is known to be involved in OMR sensorimotor processing \citep{Chen2018-sdf}, as well as turn direction regulation through the ARTR \citep{Dunn2016-asd}. 
Therefore, the reduced OMR turning responses in \emph{atp1a3a} mutants may be related to dysfunction in these areas. 
Similar to the DF stimulus, we observed strong patterns of both activation and suppression in the forebrain, including the pallium, subpallium and habenula, indicating that these areas are again particularly sensitive to \emph{atp1a3a} loss even in the context of a very different stimulus and behavioural response.
...
\caption{\textbf{Dysregulated activity in the forebrain of \emph{atp1a3a} mutants in areas homologous to the basal ganglia}\\
\textbf{A)} Arena for Optomotor Response (OMR) assay, in which larvae are placed in tracks and either presented with global red light for the stimulus-free condition ("No Stim."), or with moving striped patterns that elicit robust directional swimming responses in WT larvae ("OMR Stim.").
...
Based on this limited sampling of behavioural phenotypes, it is difficult to determine how these different phenotypes may relate to each other, and whether they are related to dysfunction in the same or different brain areas and circuits.
...
To examine the consistencies in these patterns more closely, we directly compared the MAP-Mapping results across these three experiments by generating an intersection map, where only pixels that show significant alterations in all three experiments are retained (\autoref{fig:5}A).
This revealed a very striking and bilaterally symmetical pattern of altered activity, with a zone of co-activation in the habenula (HB\textsuperscript{A}), activation of the lateral pallium (Pa\textsuperscript{A}), and two zones of activation and suppression in the subpallium  (SP\textsuperscript{A} and SP\textsuperscript{S}, respectively). 

In zebrafish, including larval stages, the habenula is thought to functions as a key integrative hub linking sensory inputs and internal state to downstream neuromodulatory circuits, thereby regulating aversive processing, stress coping, attention, and action selection \citep{Okamoto2021}. 
...
Therefore, altered habenular activity in \emph{atp1a3a} mutants may contribute to the altered stimulus responses and hyperactivity phenotypes through dysregulated sensory processing, but as its roles are thought to be higher-order and modulatory, we have focused here on the telencepahlic patterns.
...
\subsection{Ca2\textsuperscript{2+} imaging identifies altered sensorimotor responses in the \emph{atp1a3a} mutant forebrain}
...
{Number of functionally categories ROIs identified in Z-brain atlas regions.
\textbf{A)} 
...
While pERK is a useful proxy for neuronal activity, it is an indirect measure that reflects the integrated activity of neurons over a timescale of minutes, and thus does not capture the fast dynamics of neural activity that are critical for sensorimotor processing.
...
Larvae were imaged while head=embedded in agarose with the tail free to move and imaged with a modified \emph{pi\_tailtrack} apparatus \citep{Randlett2023}, integrating a videoprojector in order to present visual stimuli onto a screen below the larvae (\autoref{fig:6}A). Larvae were imaged over a 15-minute period, in a volume covering the areas of the forebrain showing the consistent altered activity patterns observed using pERK (\autoref{fig:5}, \autoref{fig:6}B). 
During imaging, larvae were presented with alternating DFs and 10 second duration pulses of forward OMR motion, repeated at 30 second intervals, to determine how these sensory stimuli are represented in the forebrain of mutants relative to siblings (\autoref{fig:6}C,D). 
...
Therefore, we also attempted to determine how these abnormal motor patterns relate to the altered forebrain activity patterns in mutants by identifying neurons that are tuned to either swimming vigor, or longer-term dynamics in the average tail angle.

To analyze the Ca\textsuperscript{2+} imaging data, we segmented the imaging data into ROIs using suite2p \citep{Pachitariu2016}, which mostly (but not exlusively) identifies individual neuronal soma, here referred to as identified "neurons". Using regressor-based analyses \citep{Miri2011} and affinty-propagation clustering \citep{Lamire2023-he}, we identified neurons that show significant responses to the DF and OMR stimuli, neurons that show significant correlations with either swimming vigor or tail angle, as well as any neurons showing significant activity that we couldnt associate with any behavioural variables (\autoref{fig:6}C-E).
...
Interestingly, there was a clear regional-dependence to this change, where the telencephalic pallium and subpallium, as well as the habenula showed a near complete loss of sensory-responsive neurons (\autoref{fig:S6}A,B).  
The other diencephalic regions we analyzed (pretectum, thalamus, posterior tuberculum) showed strong but less extreme reductions in these nueronal populations, while the midbrain tectum showed the least reduction, which did not reach statistical significance. These analyses are consistent with our MAP-Mapping experiments, showing that the \emph{atp1a3a} mutants telencephalon and habenula show the strongest alterations in physiological response, indicating that these circuits are particularly vulnerable to \emph{atp1a3a} loss, and may be critical for understanding the circuit-level mechanisms underlying the behavioural phenotypes in these mutants.
