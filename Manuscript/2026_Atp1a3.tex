...
Larval zebrafish are a powerful model for genetics and systems neuroscience, but as a very immature and rapidly developing non-mammalian vertebrate, there are some limitations to their use as models for human neurological diseases.
...
It may not be possible to answer this question \emph{a priori}, but we propose that the first and most relevant step is a thorough analysis of the behavioual phenotypes present in the model. 
This can also be complemented by neurological anlayses to determine loci of physiological modification in the brain. 
...
Proof of concept for the relevance of zebraifsh neurological diesase modelling comes from Dravet syndrome, where \emph{scn1lab} mutant larvae show spontaneous seizure-like behavior and epileptiform brain activity, and they often show poor response to sodium-channel blockers, thus showing high phenotypic similarity to the human condition \citep{Baraban2013-NC, Dinday2015-EN}. 
Large-scale larval screens identified clemizole and later lorcaserin as seizure-suppressing compounds that act via serotonergic pathways, eventually leading to clinical trials and motivating human use of serotonergic agents, including fenfluramine, which has been approved as a clinical treatment \citep{Lagae2019-LS, Nabbout2020-JAMA}. 
...
The clearest parallels between what we have observed and the human condition is the presence of postural problems, which are a hallmark of AHC and RDP, and are often triggered by specific sensory stimuli or stressors. 
While this awaits a more comprehensive understanding of the neurological basis, it is tempting to speculate that the "unbalanced swimming" could be understood as a model of ataxic behaviour, while the "bent tail" phenotypes could be framed as a model of dystonic postures. 
Or, perhaps rather hemiplegic postures, given that the tail is often persistently bent to one side for many seconds which could either indicate lateralized contraction or conversely lateralized paralaysis.
...
Symptomatic transience is one of the major hallmarks of ATP1A3-related disorders, where patients often show periods of normal or near-normal function interspersed with periods of severe symptoms, which can be triggered by specific sensory stimuli or stressors. 
Symptoms can also be relieved by sleep, and thus show a strong dependence on internal state.
We have shown that \emph{atp1a3a} mutants also show transience in their phenotypes, with periods of normal swimming interspersed with periods of unbalanced swimming and persistent tail bends.
We have not yet explored the possible triggers or releivers of these phenotypes, but the "bent tail" phnoetypes that we observe in head-embedded larvae appear to be much more extreme than those we observe in free-swimming larvae, which are much rarer and more interemittent. 
There are many established stressors used in larval zebrafihs, ranging from mechanical stimulation, light changes/strobing, temperature/heat-shock, and chemical stressors such as salt, cadaverine, or alarm pheraone. 
If fugure analyses are able to identify stressors that can trigger the postural phenotypes in \emph{atp1a3a} mutants, this may provide an invaluable model for understanding the mechanisms of symptom transience in ATP1A3-related disorders, and for screening for drugs that can prevent or relieve these catastrophic symptoms.

\subsection{Understanding the circuit-level pathologies in \emph{atp1a3a} mutants and their implications for disease}



\subsection{Conclusions}

-- atp1a3b and its potential roles need to be clarified
-- point mutations
-- cell types
-- drugs and genetic modifiers
