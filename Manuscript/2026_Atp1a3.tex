...
Mutations in ATP1A3cause a spectrum of severe neurological disorders that commonly feature episodic and stress-triggered motor symptoms, including dystonia, ataxia, and hemiplegic attacks.
ATP1A3 is the $\alpha$3 subunit of the Na$^+$/K$^+$-ATPase, which is critical for maintaining inonic homeostasis in neurons. 
...
To identify affected circuit substrates, we performed whole-brain functional imaging and found a forebrain network exhibiting pronounced dysregulation, including reduced stimulus tuning, increased motor tuning, and an abnormal bursting activity pattern among a large proportion of neurons in the mutant forebrain.
...
\textbf{A)} The larva displays strong unbalanced swimming, spending prologed periods immobile laying on its side on the bottom of the dish, and when swimmingfailing to maintain dorso-ventral alignment and swaying from side to side.
...
As the tail-bending phenotype was difficult to quantify in this multi-well plate format, we used a head-embedded preparation in which the larva is embedded in agarose with the tail free to move, and presented visual stimuli to elicit swimming behaviour using a projector and screen below the larvae (\autoref{fig:1}F).
Observing \emph{atp1a3a} mutants in this setup revealed periods of normal swimming interspersed with periods of persistent tail bends, with the tail often stably bent to one side for many seconds (\autoref{fig:1}G).
Tail tracking and swimming kinematic analyses \citep{Randlett2023} confirmed that WT sibling fish exhibit the typical bouting swimming pattern, with variable levels of activity in different fish, but that the tail angle remains stably centered along the midline (\autoref{fig:1}H).
...
Dramatically, these larvae show many periods of persistent lateral tail bending reflected as large deviations in the tail angle from the midline, lasting many seconds or minutes (\autoref{fig:1}G-I).
...
To test this hypothesis directly at the level of individual neurons, we combined HCR labelling of \emph{npy} and \emph{penkb} with pERK immunostaining in the same larvae, and quantified the pERK signal within individual marker-labelled neurons (\autoref{fig:5}F).
...
This confirmed the predictions of the atlas-based analysis: \emph{npy}-labelled neurons of the dEN/pallidum homolog showed significantly increased pERK signal in mutants (\autoref{fig:5}G), while \emph{penkb}-labelled neurons of the Vd/Vc striatum homolog showed significantly decreased pERK signal (\autoref{fig:5}H).
...
For the atlas-based comparisons in \autoref{fig:5}B-E, image volumes were registered to the Z-Brain atlas \citep{Randlett2015} using CMTK as described above.

To quantify pERK signal within individual marker-labelled neurons (\autoref{fig:5}F-H), analyses were performed in ImageJ/Fiji \citep{Schindelin2012-hd}.
