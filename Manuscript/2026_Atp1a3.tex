...
\subsection{\emph{atp1a3a} mutant phenotypes under spontaneous conditions}

Having established that \emph{atp1a3a} mutants exhibit clear behavioural abnormalities, we sought to more systematically characterize these phenotypes under spontaneous conditions, and to identify potential underlying neural circuit dysfunctions. For this we observed larvae in a free-swimming setup in which they were placed in individual wells and recorded for 30 minutes under lit-conditions \citep{Randlett2019-fj, Lamire2023-he}. 
...
Under these conditions, we did not observe significant phenotypes in heterozygous \emph{atp1a3a\textsuperscript{+/-}} larvae. 

In order to identify potential neural substrates and brain areas involved in these abnormal swimming phenotypes, we decided to map the brain-wide activity of \emph{atp1a3a} mutants relative to their WT siblings using MAP-Mapping \citep{Randlett2015}, which uses immunostaining of pERK-levels as a proxy for neuronal activity and statistical analysis to identify regions with altered activity.
We placed cohorts of 200 larvae in a 10cm petri dish to swim freely in a brightly lit environment. 
We then fixed the cohorts and stained them with antibodies against phospho-ERK (pERK) and total-ERK (tERK).
...
These maps revealed significant activation in motor-areas, including the spinal cord and midbrain tegmentum, consistent with the hyperactivity or pheraps dysregulated motor coordination in these mutants (\autoref{fig:1}C,D).
However, much more prominent alterations in pERK/tERK signals were seen in the forebrain, including areas, including the pallium, subpallium and habenula, which are not typically thought of as primary motor areas, and so their potential relationships with hyperactivity was not clear. 

Comparing the whole-brain neural activity projection map (Figure 11.H), we found highly localized changes in neural activity between mutant and wild-type groups. We observed more neural activation (green signal) in atp1a3a-/- in areas in the forebrain that correspond to the pallium (P), subpallium (SP, more ventro-lateral) and habenula (Hb), in the midbrain where the tegmentum (Teg) is localized, and in the spinal cord (SC).
